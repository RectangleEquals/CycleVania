...
| `dial(name: String)` | `float` | final | Read a dial. Inherits OUTWARD-IN and an inner scope wins, so 'set saturation once at World scope' works. The trace records which scope supplied the value. |
...
A tunable the generator may retune within declared bounds. value(ctx), min, max, set, and a source that is either a Constant or a Curve row read at a ProgressionAxis.
