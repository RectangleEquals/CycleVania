...
//! boss-then-sanctum rhythm three Reaches running and then does not has found a bug, not variety. So
...
///
/// A slot's *name* is the dev's word: `"capstone"`, `"sanctum"`, `"antechamber"`, `"the bit with the
...
mod tests {
    ...
    /// CrawlStar's loop: start → <anything> → capstone(boss) → terminal(sanctum, an exit of capstone).
    fn crawlstar_spine() -> SpineTemplate {
        SpineTemplate::new(oid("spine", "reach_loop"), NodeKind::Reach)
            ...
            .slot(
                SpineSlot::new("terminal")
                    .role(SlotRole::Goal)
                    .must_contain([oid("actor", "obelisk")])
    ...
    fn adherence_never_weakens_a_required_slot() {
        ...
        let spine = crawlstar_spine().adherence(0.0);
        assert_eq!(
    ...
    fn validation_reports_a_budget_shortfall_with_the_arithmetic() {
        let registry = registry_with(&[("actor", "boss"), ("actor", "obelisk")]);
        let spine = crawlstar_spine();
    ...
    fn validation_catches_missing_content_and_bad_references() {
        // Nothing registered: the boss and obelisk demands cannot be met.
        let v = crawlstar_spine().validate(&ContentRegistry::new(), 10);
        assert!(v
            .errors
            .iter()
    ...
    fn the_terminal_is_directly_connected_to_the_capstone() {
        // CrawlStar's requirement: the Rest Sanctum is an exit of the boss chamber.
        let (g, _) = world(1, 6);
        let mut mission = MissionGraph::new(g.of_kind(NodeKind::Space).next().unwrap().0);
        let instances = SpineInstantiator::new(&g)
            .with_template(crawlstar_spine())
    ...
    fn coverage_limits_which_instances_receive_the_spine() {
        let (g, _) = world(6, 5);
        let spine = crawlstar_spine().coverage(Coverage::Every(3));
        let mut mission = MissionGraph::new(g.of_kind(NodeKind::Space).next().unwrap().0);
        let instances = SpineInstantiator::new(&g)
            .with_template(spine)
            .instantiate(&mut mission, &Rng::new(1));
