...
/// A per-Reach loop: `start → <anything> → capstone(boss) → terminal(treasury)`, where the terminal
...
fn a_dead_end_slot_survives_free_form_growth_and_maximum_cycle_density() {
    ...
    // is the most aggressive setting there is, and it must not touch the treasury.
    ...
    assert!(
        ...
        "the treasury's one connection must be the capstone itself"
    );
...
#[test]
fn a_dead_end_treasury_does_not_strand_anyone() {
