/**
 * Dual contouring — extract the field's 0-isosurface into a mesh. One QEF-placed
 * vertex per surface-straddling cell (sharp features), quads across sign-changing
 * grid edges, normals facing the OPEN interior. Hermite normals are quantized
 * BEFORE the QEF solve (the faceting trick — co-planarizes vertices into clean
 * facets). `dualContourSteps` is a resumable generator (yields per z-slab) for
 * async slicing; `dualContour` drains it. Deterministic (our QEF + trig).
 */

import { clamp, solveQEF, type Vec3 } from "../math/index.js";
import { sdfNormal, type Sdf } from "../volume/sdf.js";
import type { FieldExtents } from "../volume/field.js";
import { snapNormal, type FidelityProfile } from "./fidelity.js";
...
export function* dualContourSteps(field: Sdf, extents: FieldExtents, profile: FidelityProfile): Generator<{ z: number; zMax: number }, Mesh, unknown> {
  const origin = extents.origin;
  const res = extents.res;
  const dx = extents.dims[0];
  const dy = extents.dims[1];
  const dz = extents.dims[2];
  const nx = dx + 1;
  const ny = dy + 1;
  const nz = dz + 1;
    ...
    for (let j = 0; j < dy; j++)
      for (let i = 0; i < dx; i++) {
        ...
        for (const [a, b] of EDGES) {
          ...
          pts.push(cp);
          // quantize the hermite normal BEFORE the QEF solve (faceting)
          nrms.push(snapNormal(sdfNormal(field, cp, res * 0.5), profile));
        }
        ...
        const nrm = snapNormal([ax / nl, ay / nl, az / nl], profile);
...
export function dualContour(field: Sdf, extents: FieldExtents, profile: FidelityProfile): Mesh {
  const g = dualContourSteps(field, extents, profile);
  let r = g.next();
